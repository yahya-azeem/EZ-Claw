...
use super::permissions::{Permission, SecurityPolicy, ToolPermissions};
...
impl SandboxManager {
    ...
    pub fn check_request(&mut self, request: &ToolExecutionRequest) -> SecurityCheckResult {
        ...
        for capability in &request.required_capabilities {
            if self.security_policy.is_denied(&request.tool_name, capability) {
                self.log_audit(&request.tool_name, "denied", &format!("Capability '{}' denied", capability));
                return SecurityCheckResult::rejected(
                    &format!("Tool '{}' does not have '{}' permission", request.tool_name, capability),
                );
            }

            if self.security_policy.requires_confirmation(&request.tool_name, capability) {
        ...
        if request.required_capabilities.iter().any(|c| c == "filesystem") {
            match self.policy {
...
impl WasmSandboxManager {
    ...
    #[wasm_bindgen]
    pub fn store_credential(&mut self, id: &str, encrypted_hex: &str) {
        self.inner.credential_vault.store_encrypted(id, encrypted_hex);
    }
