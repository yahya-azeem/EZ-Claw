...
use crate::security::permissions::AutonomyLevel;
use crate::security::prompt_defense::PromptDefense;
use crate::security::sandbox::{SandboxManager, SecurityCheckResult, ToolExecutionRequest};
use crate::text;
...
pub fn build_tool_instructions_text(tools: &[ToolSpec]) -> String {
    ...
    instructions
        .push_str("After tool execution, results appear in <tool_result> tags. ");
    instructions
...
impl WasmAgent {
    ...
    pub fn format_tool_result(
        ...
        if let Some(obj) = val.as_object_mut() {
            obj.insert("tool_call_id".into(), serde_json::Value::String(tool_call_id.into()));
    ...
    #[wasm_bindgen]
    pub fn check_tool_security(&mut self, tool_name: &str, arguments: &str, target_url: &str) -> String {
        let tool_def = self.tool_registry.get(tool_name);
        let capabilities = tool_def
    ...
    #[wasm_bindgen]
    pub fn add_credential(&mut self, mapping_json: &str, encrypted_value: &str) -> Result<(), JsValue> {
...
impl WasmAgentLoop {
    ...
    pub fn new(max_iterations: u32) -> Self {
        Self {
            ...
            iteration: 0,
            max_iterations: if max_iterations == 0 { 25 } else { max_iterations },
    ...
    pub fn begin_iteration(&mut self) -> bool {
        ...
        self.emit_event(AgentEventType::IterationProgress, &serde_json::json!({
