...
impl VirtualFS {
    ...
    fn navigate_mut(&mut self, path: &str) -> Option<&mut FSNode> {
        let parts: Vec<String> = Self::split_path(path).into_iter().map(|s| s.to_string()).collect();
    ...
    pub fn write_file(&mut self, path: &str, content: &str) -> Result<(), String> {
        let norm = Self::normalize_path(path);
        let parts: Vec<String> = Self::split_path(&norm).iter().map(|s| s.to_string()).collect();
    ...
    pub fn import(&mut self, json: &str) -> Result<(), String> {
        self.root =
            serde_json::from_str(json).map_err(|e| format!("Invalid FS JSON: {}", e))?;
        Ok(())
    }
}
...
impl WasmWorkspace {
    ...
    pub fn read_file(&self, path: &str) -> Result<String, JsValue> {
        self.inner.read_file(path).map_err(|e| JsValue::from_str(&e))
    }
