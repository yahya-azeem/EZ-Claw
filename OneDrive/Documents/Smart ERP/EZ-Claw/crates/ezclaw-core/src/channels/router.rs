...
use super::types::{ChannelType, ChannelMessage, ChannelConfig};
...
impl RouteKey {
    ...
    pub fn to_string_key(&self) -> String {
        format!("{}:{}:{}", self.channel_type, self.channel_id, self.sender_id)
    }
}
...
impl ChannelRouter {
    ...
    pub fn register_channel(&mut self, config: ChannelConfig) {
        self.configs.insert(config.channel_type.as_str().to_string(), config);
    }
...
impl WasmChannelRouter {
    ...
    pub fn import_state(&mut self, json: &str) -> Result<(), JsValue> {
        self.inner = ChannelRouter::from_json(json)
